...
import com.dicoding.picodiploma.loginwithanimation.data.retrofit.StoryPagingSource
...
class UserRepository private constructor(
    ...
    fun getStoriesWithLocation(token: String): LiveData<Result<List<ListStoryItem>>> = liveData(Dispatchers.IO) {
        emit(Result.Loading)
        try {
    ...
    companion object {
        ...
        fun getInstance(apiService: ApiService, userPreference: UserPreference, localeDataStore: LocaleDataStore) =
            instance ?: synchronized(this) { UserRepository(apiService, userPreference, localeDataStore) }.also { instance = it }
    }
